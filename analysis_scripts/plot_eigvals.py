...
def extract_trained_model_info(state_dict, model_dir) -> "Tuple[int, int, bool, int, int]":
...
def get_trained_dae_model_from_pt(model_path: str, cfg: dict, task: str, dt: float = 0.02, device: torch.device = torch.device('cpu')):
    ...
    is_exported = any("obs_fn" in k and k.endswith(".weight") for k in cleaned_state_dict.keys())
...
def get_trained_dae_model(model_dir):
    ...
    _, G = load_symmetric_system(robot_name="a1")
...
def initialize_dae_model(cfg, task: str, dt: int, device: torch.device) -> torch.nn.Module:
    ...
    try:
        # Define the state representation
        # G is the symmetry group of the system
        _, G = load_symmetric_system(robot_name=cfg["robot"]["name"])
...
def main():
    ...
    # Base data path for the new structure
    base_data_path = "../hf_datasets/training_data"

    # Explicit ordering for tasks (moved up to control directory scanning)
    ...
    for task_name in ordered_tasks:
        task_dir = os.path.join(base_data_path, task_name)
        if not os.path.exists(task_dir):
            print(f"[Warning] Task directory not found, skipping: {task_dir}")
            continue

        # Find experiment directories (e.g., matching {task_name}*)
        experiment_dirs = sorted([d for d in os.listdir(task_dir) if os.path.isdir(os.path.join(task_dir, d))])

        for exp_name in experiment_dirs:
            # Determine DAE type from the folder name
            if "ecdae" in exp_name.lower():
                dae_type = "ecDAE"
            elif "cdae" in exp_name.lower():
                dae_type = "cDAE"
            else:
                continue

            key = (task_name, dae_type)
            unique_types.add(dae_type)
            if key not in collected_results:
                collected_results[key] = { 'max_eig': [], 'cond_num': [], 'rank': [], 'state_dim': [], 'runs': [] }

            exp_dir_path = os.path.join(task_dir, exp_name)

            # Get all run subdirectories starting with '202' and sort them alphabetically
            run_subdirs = sorted([d for d in os.listdir(exp_dir_path) if d.startswith("202") and os.path.isdir(os.path.join(exp_dir_path, d))])
            if not run_subdirs:
                print(f"[Warning] No run subdirectories starting with '202' found in {exp_dir_path}")
                continue

            for run_subdir in run_subdirs:
                run_dir_path = os.path.join(exp_dir_path, run_subdir)

                # Find the dae_model_*.pt files
                ...
                try:
                    latest_model_path = max(model_files, key=lambda f: int(re.search(r'dae_model_(\d+).pt', os.path.basename(f)).group(1)))
                except Exception as e:
                    print(f"[Warning] Error parsing model iteration numbers in {run_dir_path}: {e}")
                    continue
                ...
                # Extract matrices, eigenvalues, singular values, and rank
    ...
    for j, task_name in enumerate(ordered_tasks):
        for k, dae_type in enumerate(sorted_unique_types):
            key = (task_name, dae_type)
            if key in collected_results:
                ...
                if res['runs']: # ensure some seeds were actually found
                    ...
                    # Assumption: state_dim is the same for all seeds of a task/type combo
                    state_dim_for_label = res['state_dim'][0]

                    # Ensure no underscores make it to LaTeX and print bolded task only on the first iteration
                    pretty_task_name = task_display_names.get(task_name, task_name)
                    display_task = f"\\textbf{{{pretty_task_name}}}" if k == 0 else ""
    ...
    for i, task_name in enumerate(ordered_tasks):
        for j, dae_type in enumerate(sorted_unique_types):
            ...
            if key in collected_results and collected_results[key]['runs']:
                # The prompt included a debug input here for testing purposes
                if key == ('walk_slope', 'cDAE'):
                    last_run_tuple = collected_results[key]['runs'][-3]
                else:
                ...
                print(f"[Plotting] Generating plots for: {model_file_plot}")
    ...
    # Step 4: Final logic for y-limits on singular values and saving all figures
    ...
    fig_a.savefig("paper_plots/all_a_matrices_last_seed.pdf", bbox_inches='tight', pad_inches=0.01)
    fig_b.savefig("paper_plots/all_b_matrices_last_seed.pdf", bbox_inches='tight', pad_inches=0.01)
    fig_eig.savefig("paper_plots/all_a_eigenvalues_last_seed.pdf", bbox_inches='tight', pad_inches=0.01)
    fig_eig_mag.savefig("paper_plots/all_a_eigenvalue_magnitudes_last_seed.pdf", bbox_inches='tight', pad_inches=0.01)
    fig_sv.savefig("paper_plots/all_controllability_svs_last_seed.pdf", bbox_inches='tight', pad_inches=0.01)
    plt.close(fig_a)
    plt.close(fig_b)
    plt.close(fig_eig)

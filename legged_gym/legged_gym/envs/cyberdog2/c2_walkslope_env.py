...
class CyberWalkSlopeEnv(CyberEnv):
    def __init__(self, cfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.cam_env_id = torch.where(self.terrain_levels == 4)[0][0] # get the env with terrain level 4
        self.setup_camera()
        if self.headless:
            camera_position = gymapi.Vec3(-1. + self.env_origins[self.cam_env_id, 0], 0. + self.env_origins[self.cam_env_id, 1], 1. + self.env_origins[self.cam_env_id, 2])
            camera_target = gymapi.Vec3(10. + self.env_origins[self.cam_env_id, 0], 0. + self.env_origins[self.cam_env_id, 1], 0. + self.env_origins[self.cam_env_id, 2])
    ...
    def setup_camera(self):
        ...
        self.camera_handle = self.gym.create_camera_sensor(self.envs[self.cam_env_id], camera_props)

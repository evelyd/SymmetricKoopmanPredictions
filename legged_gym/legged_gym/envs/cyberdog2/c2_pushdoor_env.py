...
class CyberPushDoorEnv(CyberEnv):
    ...
    def post_physics_step(self):
        ...
        # Compute metrics for CoT
        if self.cfg.mode == "test":
            # Power = |tau * dof_vel|, Energy = Power * dt
            power = torch.sum(torch.abs(self.torques * self.dof_vel), dim=-1)
            self.env_energy_buf += power * self.dt
        ...
        if len(env_ids) > 0 and self.cfg.mode == "test":
    ...
    def collect_test_stats(self, env_ids):
        ...
        if hasattr(self, "test_stats_done") and self.test_stats_done:
            return

        # Check for success condition only for the environments that are resetting
        success_condition = torch.logical_and(
            torch.logical_or(
                torch.logical_and(self.root_states[env_ids, 0] - self.env_origins[env_ids, 0] > 2, self.door_dof_pos[env_ids, 0] > np.pi / 3),
                self.door_dof_pos[env_ids, 0] > np.pi / 2 - 0.1),
            self.episode_length_buf[env_ids] > 150
        )
        successful_envs = env_ids[success_condition]

        if len(successful_envs) > 0:
            # 1. Time to Task Completion (seconds)
            ttc = self.episode_length_buf[successful_envs] * self.dt
            self.total_success_ttc += torch.sum(ttc).item()

            # 2. Cost of Transport (approximate mass of CyberDog2 = 15kg)
            mass = 15.0 
            g = 9.81
            distances = torch.norm(self.root_states[successful_envs, :2] - self.env_origins[successful_envs, :2], dim=-1)
            distances = torch.clamp(distances, min=1e-3) # Prevent division by zero
            
            cot = self.env_energy_buf[successful_envs] / (mass * g * distances)
            self.total_success_cot += torch.sum(cot).item()

        # Increment success buffer for these environments
        self.env_success_buffer[env_ids] += success_condition
        self.env_finish_buffer[env_ids] += 1

        # print(f"finish buffer: {self.env_finish_buffer}, total: {torch.sum(self.env_finish_buffer)}, success buffer: {self.env_success_buffer}, total: {torch.sum(self.env_success_buffer)}")
        print(f"finished: {torch.sum(self.env_finish_buffer)} / 10000")
        # Check if all environments have finished a sufficient number of episodes
        if torch.sum(self.env_finish_buffer) >= 10000: #TODO tune this quantity
            self.finish_count = torch.sum(self.env_finish_buffer).item()
            self.success_count = torch.sum(self.env_success_buffer).item()
    ...
    def reset_idx(self, env_ids):
        ...
        if self.cfg.mode == "test":
            self.env_energy_buf[env_ids] = 0.0
    ...
    def _create_envs(self):
        ...
        if not self.headless:
            # 创建相机
            self.camera_handle = self.gym.create_camera_sensor(self.envs[self.cam_env_id], gymapi.CameraProperties())
            camera_position = gymapi.Vec3(-0.5 + self.env_origins[self.cam_env_id, 0],
                                            0.5 + self.env_origins[self.cam_env_id, 1],
                                            1 + self.env_origins[self.cam_env_id, 2])
            camera_target = gymapi.Vec3(0.5 + self.env_origins[self.cam_env_id, 0],
                                        0. + self.env_origins[self.cam_env_id, 1],
                                        0.5 + self.env_origins[self.cam_env_id, 2])
            self.gym.set_camera_location(self.camera_handle, self.envs[self.cam_env_id], camera_position, camera_target)
    ...
    def _init_buffers(self):
        ...
        if self.cfg.mode == "test":
            ...
            self.env_energy_buf = torch.zeros((self.num_envs,), dtype=torch.float, device=self.device)
            self.total_success_ttc = 0.0
            self.total_success_cot = 0.0
            self.test_stats_done = False

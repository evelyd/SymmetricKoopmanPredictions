...
class CyberStandDanceEnv(CyberEnv):
    def __init__(self, cfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.setup_camera()
        if self.headless:
            camera_position = gymapi.Vec3(1.0+self.env_origins[self.cam_env_id, 0], 1.0 + self.env_origins[self.cam_env_id, 1], 0.5 + self.env_origins[self.cam_env_id, 2])
            camera_target = gymapi.Vec3(0.0+self.env_origins[self.cam_env_id, 0], 0.0 + self.env_origins[self.cam_env_id, 1], 0.5 + self.env_origins[self.cam_env_id, 2])
    ...
    def post_physics_step(self):
        ...
        self.compute_reward()
        # print("Mean reward:", self.rew_buf.mean().item())
